...
import { Printer, Camera, Plus } from "lucide-react";
...
// Handles both new (no wo.id) and existing work orders.
    ...
    <div style={overlay} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: 680, maxWidth: "96vw" }}>
        ...
        <div id="wo" style={{ background: "#fff", border: `1px solid ${C.line}`, padding: "30px 34px" }}>
          {t === "sewing" ? <SewingBody {...bodyProps} />
            : t === "saw" ? <SawBody {...bodyProps} />
            : t === "cnc" ? <CncBody {...bodyProps} />
            : <BasicBody {...bodyProps} />}
        </div>
      </div>
    </div>
...
function EI({ value, onChange, size = 16, bold, mono, width, align = "left", placeholder, full }) {
  return (
    <input
      className="wo-edit"
      value={value || ""}
...
// Read-only order number (auto-assigned, shared with the orders sequence).
function ONo({ orderNo }) {
  return <span className="font-bold" style={{ fontFamily: "ui-monospace,monospace", fontSize: 14 }}>{orderNo ? `#${orderNo}` : "—"}</span>;
}
...
// ---- Shop (basic) ----
function BasicBody({ fields, set, orderNo }) {
  return (
    <>
      <div className="flex items-start justify-between" style={{ marginBottom: 18 }}>
        ...
        <div style={{ textAlign: "right", minWidth: 200 }}>
          <RowEdit label="Order #"><ONo orderNo={orderNo} /></RowEdit>
...
function CncBody({ fields, set, orderNo }) {
...
function SewingBody({ fields, set, setLineCell, addLine, form, orderNo }) {
...
function SawBody({ fields, set, setLineCell, addLine, form, orderNo }) {
