  ...
  const ins = await fetch(`${url}/rest/v1/rpc/create_order`, {
    ...
    body: JSON.stringify({
      p_order: {
        ...
        source: "Shopify",
        will_call: false,
        // Shopify rule: every order is due 5 days after it was placed.
...
// chars are the local placed-date; we add 5 days with plain date math (anchored
// at UTC midnight) to avoid any timezone drift. Returns "YYYY-MM-DD".
